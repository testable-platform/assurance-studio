...
from sa_metrics import SA_TESTING_TYPE, report_folder_name, report_output_dir  # noqa: E402
...
def main():
    ...
    for idx, branch_name in enumerate(branches, start=1):
        ...
        try:
            ...
            report_folder = save_report(
                output_dir, branch_name, run_id, summary, taxonomy_json, taxonomy_xml)
            ...
            print("  saved to %s (status=%s, gate=%s)" % (
                os.path.join(output_dir, report_folder), entry["status"], entry["gate_score"]))
        except Exception as exc:
            print("  FAILED: %s" % exc)
